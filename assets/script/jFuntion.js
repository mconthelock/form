 ...
 * @returns {string} The HTML string of the fieldset element.
 * @example
 * const fieldsetHTML = fieldAddInput('<input type="text" class="input" />', 'My Label');
 * console.log(fieldsetHTML);
 * // Output:   <fieldset class="fieldset w-full">
 *                  <label class="fieldset-label">My Label</label>
...
export const fieldAddInput = (inputElement, label = "") => {
  const labelElement =
    label == "" ? "" : `<label class="fieldset-label">${label}</label>`;
  return `<fieldset class="fieldset w-full">
                ${labelElement}
                <div class="flex gap-1">
                    <div class="flex flex-col flex-1 inputGroup gap-1 w-full">
                        ${inputElement}
 ...
 */
export async function resetInput(
  input,
  button = ".remove-input",
  clear = false
) {
