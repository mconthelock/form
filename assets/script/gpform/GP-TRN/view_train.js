...
$(document).ready(async function () {
    ...
    $(document).on("change", "#viewDateFrom, #viewDateTo", function () {
        ...
        if ($("#viewDateFrom").val() && $("#viewDateTo").val() && to < from) {
            Swal.fire({ icon: "warning", title: "⚠ กรุณาเลือกวันที่ให้ถูกต้อง" });
    ...
    const flow = await showFlow(formData.nfrmno, formData.vorgno, formData.cyear, formData.cyear2, formData.nrunno);
    $(".flow").html(flow.html);
    ...
    window.confirmDelete = function (filename, cyear2, nrunno, type_att, id) {
        Swal.fire({
            ...
            if (res.isConfirmed) deleteAttachment(filename, cyear2, nrunno, type_att, id);
        });
    };

    window.deleteAttachment = function (filename, cyear2, nrunno, type_att, id) {
        ...
        $.post(`${baseUrl}gpform/GP-TRN/training/delete_attachment`, {
            ...
            .done(() => {
                Swal.fire({ icon: "success", title: "ลบไฟล์สำเร็จ", timer: 1200, showConfirmButton: false });
                location.reload();
            })
            .fail(() => Swal.fire({ icon: "error", title: "ไม่สามารถลบไฟล์ได้" }));
    };
