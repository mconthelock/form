...
async function handleFormSubmit(formType, fid) {
  ...
  switch (formType) {
    case "functional":
      ...
      reqby = [{ code: getVal("funcTraineeCode"), pos: getVal("funcTraineeposcode") || "" }];
    ...
    case "meth":
      ...
      reqby = [{ code: getVal("methTraineeCode"), pos: getVal("methTraineeposcode") || "" }];
      inputby = getVal("methInputBy");
      break;

    case "pos":
      isValid = validatePosForm();
      reqby = [{ code: getVal("posTraineeCode"), pos: getVal("posTraineeposcode") || "" }];
...
async function submitForm(formType, reqby, inputby, fid) {
  try {
    ...
    if (formType === "legal" || formType === "out") {
        ...
        const resGroup = await fetch(`${host}gpform/GP-TRN/training/get_new_group_train`, {
          method: "POST",
