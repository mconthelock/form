...
exports.validateFunctionalForm = validateFunctionalForm;
exports.validateLegalForm = validateLegalForm;
exports.validateMethForm = validateMethForm;
...
function validateForm(requiredSelectors) {
    for (const sel of requiredSelectors) {
        ...
        if (el && !el.value.trim()) {
            (0, alert_js_1.showAlert)("⚠ แจ้งเตือน", el.dataset.alert || "กรุณากรอกข้อมูลให้ครบถ้วน");
            el.focus();
            return false;
        }
    }
...
function validateFunctionalForm() {
    ...
    if (!expense) {
        (0, alert_js_1.showAlert)("⚠ แจ้งเตือน", "กรุณาเลือกวิธีการพิจารณาค่าใช้จ่าย (Part 5)");
        return false;
    }
    if (expense.value === "not_compare") {
        ...
        if (!chk_reason) {
            (0, alert_js_1.showAlert)("⚠ แจ้งเตือน", "กรุณาเลือกเหตุผลของการไม่มีการเปรียบเทียบราคา ค่าฝึกอบรม ");
            return false;
        }
        else if (reasonOther) {
            const txt = document.getElementById("funcReasonOtherText");
            if (!txt.value.trim()) {
                (0, alert_js_1.showAlert)("⚠ แจ้งเตือน", txt.dataset.alert || "กรุณาระบุเหตุผลอื่น");
                txt.focus();
                return false;
            }
        }
        ...
        if (!files || files.length === 0) {
            (0, alert_js_1.showAlert)("⚠ แจ้งเตือน", "กรุณาแนบไฟล์เปรียบเทียบราคาอย่างน้อย 1 ไฟล์");
            return false;
        }
    }
    ...
    if (!freeSelected) {
        ...
        if (amount && amount.value === "") {
            (0, alert_js_1.showAlert)("⚠ แจ้งเตือน", "กรุณากรอกจำนวนเงิน");
            amount.focus();
            return false;
        }
    }
...
function validateLegalForm() {
    ...
    if (!expense) {
        (0, alert_js_1.showAlert)("⚠ แจ้งเตือน", "กรุณาเลือกวิธีการพิจารณาค่าใช้จ่าย (Part 6)");
        return false;
    }
    if (expense.value === "not_compare") {
        ...
        if (!files || files.length === 0) {
            (0, alert_js_1.showAlert)("⚠ แจ้งเตือน", "กรุณาแนบไฟล์เปรียบเทียบราคา");
            return false;
        }
    }
