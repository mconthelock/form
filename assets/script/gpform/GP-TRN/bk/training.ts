...
function buildFormDataGeneric(
	headResult: any,
	fid: string,
	prefix: "func" | "legal" | "meth"
): FormData {
	...
	const getVal = (id: string, def: string = "") =>
		(
			document.getElementById(`${prefix}${id}`) as
				| HTMLInputElement
				| HTMLTextAreaElement
		)?.value || def;
	...
	const compareFiles = (
		document.getElementById(`${prefix}CompareFiles`) as HTMLInputElement
	)?.files;
	if (compareFiles) {
	...
	switch (prefix) {
		case "func":
			...
			const jdFiles = (
				document.getElementById(`${prefix}JdFiles`) as HTMLInputElement
			)?.files;
			if (jdFiles) {
...
document.addEventListener("DOMContentLoaded", () => {
	const trainingType = document.getElementById(
		"trainingType"
	...
	async function submitForm(
		formType: "functional" | "legal" | "meth",
		reqby: string,
		inputby: string,
		fid: string
	) {
		try {
			...
			const nfrmno =
				(
					document.getElementById("NFRMNO") as HTMLInputElement
				)?.value?.trim() || "";
			const vorgno =
				(
					document.getElementById("VORGNO") as HTMLInputElement
				)?.value?.trim() || "";
			const cyear =
				(
					document.getElementById("CYEAR") as HTMLInputElement
				)?.value?.trim() || "";
		...
		if (formType === "functional") {
			...
			reqby =
				(document.getElementById("funcRequestBy") as HTMLInputElement)
					?.value || "";
			inputby =
				(document.getElementById("funcInputBy") as HTMLInputElement)
					?.value || "";
			fid = "1";
			...
			reqby =
				(document.getElementById("legalRequestBy") as HTMLInputElement)
					?.value || "";
			inputby =
				(document.getElementById("legalInputBy") as HTMLInputElement)
					?.value || "";
			fid = "2";
			...
			reqby =
				(document.getElementById("methRequestBy") as HTMLInputElement)
					?.value || "";
			inputby =
				(document.getElementById("methInputBy") as HTMLInputElement)
					?.value || "";
			fid = "3";
		}
		if (!isValid) return;
	...
	// Event ปุ่มส่งฟอร์ม
	document
		.getElementById("sendFuncFormBtn")
		?.addEventListener("click", () => handleFormSubmit("functional"));
	document
		.getElementById("sendLegalFormBtn")
		?.addEventListener("click", () => handleFormSubmit("legal"));
	document
		.getElementById("sendMethFormBtn")
		?.addEventListener("click", () => handleFormSubmit("meth"));
