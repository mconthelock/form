...
export function buildFormDataGeneric(head, fid, prefix) {
	...
	fd.append(
		"TIME_FROM",
		getVal("TimeFromHour", "00") + getVal("TimeFromMin", "00")
	);
	fd.append(
		"TIME_TO",
		getVal("TimeToHour", "00") + getVal("TimeToMin", "00")
	);
	...
	fd.append(
		"TRN_EXPENSE_STATUS",
		$(`input[name='${prefix}ExpenseOption']:checked`).val() || ""
	);
	fd.append(
		"TRN_EXPENSE_REASON",
		$(`input[name='${prefix}Reason']:checked`).val() || ""
	);
...
export async function savedetailForm(fd) {
	...
	try {
		...
		if (json.status === "success") {
			...
			showAlert(
				"⚠ แจ้งเตือน",
				json.message || "ไม่สามารถบันทึกข้อมูลได้"
			);
		}
