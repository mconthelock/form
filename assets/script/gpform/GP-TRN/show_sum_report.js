...
window.initFormReport = async function () {
    ...
    const columns = [
        { title: "Formno", data: "FORMNO_DISPLAY", className: "dt-nowrap text-center" },
        ...
        { title: "Position", data: "SPOSITION",className: "dt-nowrap text-center" },
        ...
        { title: "From", data: "DATE_FROM_DISPLAY", className: "dt-nowrap text-center" },
        { title: "To", data: "DATE_TO_DISPLAY", className: "dt-nowrap text-center" },
        { title: "Status", data: "STATUS_TEXT",
            className: "dt-nowrap text-center",
            render: function (data) {
                if (data === "Approved") return `<span style="color:green;font-weight:bold">${data}</span>`;
                if (data === "Running")  return `<span style="color:blue;font-weight:bold">${data}</span>`;
                if (data === "Reject")   return `<span style="color:red;font-weight:bold">${data}</span>`;
                return data;
            }
        },
        { title: "Amount", data: "COST", className: "dt-nowrap text-end",
            render: $.fn.dataTable.render.number(',', '.', 0)
        },
        { title: "Vat7%", data: "VAT", className: "dt-nowrap text-end",
            render: $.fn.dataTable.render.number(',', '.', 0)
        },
        { title: "Total Amount", data: "TOTAL", className: "dt-nowrap text-end",
            render: $.fn.dataTable.render.number(',', '.', 0)
        },
    ];
    ...
    $("#btnSearchSubmit").off("click").on("click", function () {
        filterModal.close();
        loadReportData();
    });
...
window.loadReportData = function () {
    ...
    $.get(window.baseUrl + "gpform/GP-TRN/training/load_data", {
        ...
        let data = JSON.parse(res).map((row, i) => {
            ...
            let run = row.NRUNNO ? row.NRUNNO.toString().padStart(6, '0') : "";
