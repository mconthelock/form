...
$(function () {
  ...
  $("#submit-btn-noAdv").click(async function (e) {
    ...
    if (amecCount() > guestCount() && $("#remark-participant").val() == "") return showInputToast("#remark-participant", "กรณีคน Amec มากกว่ากรุณากรอก Remark");
    ...
    formDataSubmit.append("location", isLocationVisible ? $("input[name='location']:checked").val() : "");
    formDataSubmit.append("location_detail", isLocationVisible ? $("#location_detail").val() : "");
    formDataSubmit.append("guest_type", $(".guest_type:checked").val() ?? "");
    formDataSubmit.append("total_amount", $("#total-amount").text());
    formDataSubmit.append("remark", $("textarea[placeholder*='ระบุเหตุผล']").val());
    ...
    companiesArray.forEach((c, i) => {
      ...
      if (fileInput && fileInput.files.length > 0) {
        formDataSubmit.append(`company_files[${i}]`, fileInput.files[0]);
      }
    });
    ...
    $("#expense-table tbody tr").each(function () {
      const receiptFile = $(this).find("input[name='receipt_file[]']")[0];
      if (receiptFile && receiptFile.files.length > 0) {
        formDataSubmit.append(`receipt_file_${rowIndex}`, receiptFile.files[0]);
      }
      rowIndex++;
    });
  ...
  $("#input-by, #requested-by, #entertain-date, #purpose, #location_detail, #guest-name-input, #amec-name-input, #remark, #reason").on("input change", function () {
    clearFieldError(this.id);
  });
