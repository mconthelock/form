...
$(function () {
	...
	$.ajax({
		...
		success: function (response) {
			$(".estimate-type").each(function () {
				$(this)
					.empty()
					.append(`<option value="">--Select Detail--</option>`);
				response.forEach((item) => {
					$(this).append(
						`<option value="${item.ET_NAME}" id="${item.ET_ID}" data-cost="${item.ET_COST}">${item.ET_NAME}</option>`
					);
				});
			});
		},
	...
	$("#submit-btn").click(async function (e) {
		...
		if (!$(".time-radio:checked").val())
			return showRadioToast(".time-radio", "กรุณาเลือกช่วงเวลา");
		...
		$("#table_cost tbody tr").each(function () {
			...
			if (details && qty && cost && total)
				estimate_items.push({ details, qty, cost, total, remark, id });
		});
...
$(document).on("change", ".time", function () {
	...
	$select.find("option").each(function () {
		...
		if (time === "Lunch" || time === "Dinner") {
			...
			if (
				val.toLowerCase().includes("gift") ||
				val.toLowerCase().includes("other")
			) {
