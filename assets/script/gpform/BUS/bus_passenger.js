...
$(document).ready(async function () {
	...
	try {
		...
		if (BUS_LINES.length > 0) {
			...
			$("#passLineName").text(BUS_LINES[0].BUSNAME);
...
$(document).on("click", ".line-row", function () {
	...
	$("#passLineName").text(data.BUSNAME);
	showPassenger(selectedBusId);
});
...
$(document).on("click", "#btnAddEmp", function () {
	...
	const line = BUS_LINES.find(
		(l) => Number(l.BUSID) === Number(selectedBusId),
	);
