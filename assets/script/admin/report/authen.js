...
import { getOrganizations, getReportMasterAuthen } from '../../service';
import { updateReportAuthen } from './data';
...
async function reloadTable() {
    ...
    const id = $('#id').val();
    const data = await getReportMasterAuthen(id);
    if (data === null) {
        throw new Error('Form Master not found');
    }

    const dataFiltered = data.filter(
        (item) => item.SPOSCODE != '80' && item.SPOSCODE != '98',
    );
    if (!table) {
