...
import { customTooltip } from "./_tooltip";
...
export const iconfont = (textSize = "text-2xl") => {
  return {
    ...
    image: `<div class="tooltip tooltip-info" data-tip="" data-html="preview image">
 ...
 * @returns
 */
export const dragDropInit = (name, format = "image") => {
  return `<label for='${name}'  class="border border-primary border-dashed rounded-lg overflow-scroll w-full min-h-60 text-primary cursor-pointer dropZone">
    ...
    <input type="file" class="file-input txt-upper validator req hidden" data-format='${format}' name="${name}" id="${name}" multiple/>`;
};

export const dragDropReset = (
  name,
  size = "btn-sm",
  color = "btn-error",
  margin = "ml-auto"
 ...
 */
const filesData = [];
export async function handleFiles(files, element, format = fileFormats.image) {
  console.log(files, element, filesData, format);
