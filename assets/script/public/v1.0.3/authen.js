...
export async function redirectProduction(id) {
...
export async function initAuthen(options = {}) {
    ...
    if (!cookie) {
        ...
        $("#user-login").attr("groupcode", group.GROUP_CODE);
...
export function setSession(group, info, menu) {
    const submenu = menu
        .filter((m) => m.submenu && m.submenu.length > 0)
        .map((m) => m.submenu)
        .flat();
...
export async function clearAuth(id) {
