 ...
 * @param {object} data 
 * @param {object} column [{key:'AMEC_SDS_ID', header: 'AMEC SDS ID'}]
 * @param {callback} options 
 * @returns 
 */
export async function defaultExcel(data, column, options = {}) {
    const option = {...excelOptions, ...options}; // รวม options ที่ส่งเข้ามา
    console.log(data, column, option);
    
    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet(option.sheetName); // เพื่มชีท และตั้งชื่อชีท
    ...
    headerRow.font = option.font;
    headerRow.alignment = option.alignment;
    // ตรวจสอบว่ามีฟังก์ชัน customActions หรือไม่
    if (option.customActions(sheet)) {
        option.customActions(sheet); // ส่ง sheet เพื่อให้ปรับแต่งตามที่กำหนด
    }
    ...
    sheet.columns.forEach((column) => {
        ...
        column.width = maxLength + option.extraWidth; // เพิ่มความกว้างอีกเล็กน้อยเพื่อไม่ให้ข้อมูลชนขอบ
    });
    return workbook;
}
