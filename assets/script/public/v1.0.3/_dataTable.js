...
export const tableOption = {
    ...
    dom: '<"flex mb-3"<"table-search flex flex-1 gap-5 "f><"flex items-center table-option"l>><"bg-white border border-slate-300 rounded-lg overflow-hidden my-5"t><"flex mt-5"<"flex-1 table-paging"p><"table-info flex  flex-none gap-5"i>>',
    pageLength: 15,
    autoWidth: false,
    destroy: true,
    responsive: true,
...
export async function createTable(option = {}, setupOpt = {}) {
    // *** structuredClone เพื่อป้องกันการเปลี่ยนแปลงค่าเริ่มต้นของ หากเป็น callback ไม่สามารถใช้ได้
    // *** รองรับ ES2021+ (เช่น Chrome 98+, Edge 98+, Node.js 17+)
    ...
    // กันกรณีที่ผู้ใช้ส่ง id มาแบบไม่มี #
    if (setup.id.startsWith("#") === false) {
        setup.id = `#${setup.id}`;
    }
    ...
    opt.initComplete = function (settings, json) {
        ...
        // เรียกใช้ function button filter
        if (setup.buttonFilter.status) {
            setBtnFilter( api, setup.id, setup.buttonFilter.column, setup.buttonFilter.activeFilter); 
        }
        // เรียกใช้ฟังก์ชัน createColumnFilters
        if (setup.columnFilter.status) {
            createColumnFilters(api, setup.columnFilter.column); 
        }
    ...
    $('.dt-search input[type="search"]').on("input", function () {
        if (setup.columnSelect.status && setup.columnSelect.resetOnSearch) {
            ...
            if (
                selectAll[0] &&
                (selectAll[0].indeterminate || selectAll[0].checked)
            )
    ...
    $("#select-dt-rows-all").on("click", function (e) {
        ...
        // หากเลือกไม่ครบทุกแถวให้เป็น indeterminate
...
function setBtnFilter(table, id, col, active = 0) {
    ...
    if (uniqueCategories.length > 0) {
        ...
        btnFilter.find(".filter-btn-dt").eq(active).trigger("click"); // กดปุ่มแรกที่มี class btn-primary
    }
}
...
export function createColumnFilters(table, columnsToFilter) {
    ...
    $(`#${tableId}`).on("click", ".filter-icon", async function (e) {
        ...
        const columnName = `filterHeader-${tableId}-${columnIndex}`;
        const filterOptions = $(
            `#filter-options .${columnName} .filter-menu-data`
        );
