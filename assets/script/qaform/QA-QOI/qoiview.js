...
$(document).ready(async function () {
	...
	flatpickr("#expect_date", {
		dateFormat: "d/m/Y",
		defaultDate: $("#expect_date").val(),
	});
	...
	console.log(empno);
	if ($("#cextData").val() == "07")
		loadUsersToSelect($(".eng_select"), "W", empno);
	$(".flow").html(flow.html);
...
function checkData() {
	...
	if (cextdata == "02") {
		...
		$(".measure").each(function () {
			if (
				$(this).find(".m_action").val() != "" &&
				$(this).find(".m_due_date").val() != "" &&
				$(this).find(".m_in_charge").val() != ""
			) {
				m++;
				return false;
			}
		...
		$(".correct").each(function () {
			if (
				$(this).find(".c_action").val() != "" &&
				$(this).find(".c_due_date").val() != "" &&
				$(this).find(".c_in_charge").val() != ""
			) {
				c++;
				return false;
			}
