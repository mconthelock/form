...
const columns = [
	...
	{
		data: "SUBCONNAME",
		title: "Supplier or Subcontrector Name ",
		width: "15%",
	},
...
$(document).on("click", ".edit-dwg", async function (e) {
	...
	for (const [key, value] of Object.entries(data)) {
		...
		if (target.is("select")) {
			...
			files.forEach((entry) => {
				const [fileId, filename, ofilename, filetype] =
					entry.split("|");
				const iconClass =
					filetype === "D"
						? "icofont-image"
...
async function createTableDwg() {
	...
	mtable = createTable(
		...
		},
		{
			...
			buttonFilter: { status: true, column: "0" },
			join: true,
			// ใส่ initComplete ตรงนี้ได้เลยครับ!
			initComplete: function (settings, json) {
				
				// เช็คกันเหนียวว่ามีปุ่มหรือยัง
				if ($('#add-dwg').length === 0) {
					$(`#table_wrapper .table-option`).append(`
						<label for="" class="btn btn-sm flex items-center max-w-xs tooltip tooltip-left bg-transparent text-gray-800 rounded-l-md hover:bg-gray-300 hover:text-black transition-colors duration-200" data-tip="Export Excel" id="exportExcel" tableID="#table">
							<i class="icofont-file-excel text-xl"></i>
						</label>
						<label for="drawer-master" class="btn btn-sm flex items-center max-w-xs tooltip tooltip-left bg-transparent text-gray-800 rounded-r-md hover:bg-gray-300 hover:text-black transition-colors duration-200" data-tip="Add" id="add-dwg" tableID="#table">
