...
import { skeleton } from "@amec/webasset/skeleton";
import { formatDate } from "@amec/webasset/dayjs";
import { getAuditMaster } from "@amec/webasset/api/escs";
import { logtest, showErrorMessage } from "@amec/webasset/utils";
...
$(document).on("click", "#save-button", async function () {
	...
	topic.forEach((t) => {
		if (
			t.getAttribute("edit") == "true" ||
			t.getAttribute("new") == "true" ||
			t.getAttribute("del") == "true"
		) {
			data.topic.push({
	...
	list.forEach((l) => {
		if (
			l.getAttribute("edit") == "true" ||
			l.getAttribute("new") == "true" ||
			l.getAttribute("del") == "true"
		) {
			data.list.push({
