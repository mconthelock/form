 ...
 */
  const renderTable = (systems = []) => {
    ...
    const isEmpty = !systems.length || systems.every(s => Number(s.total_users) === 0);
    ...
    const html = systems.map((system, sysIdx) => {
      ...
      return programs.map((program, progIdx) => {
        ...
        return `
                <tr class="hover ${borderClass} transition-all duration-200">
                    ${isFirst ? `
                        <th rowspan="${rowspan}" class="bg-base-200/50 text-center text-lg font-black border-r border-base-300 w-12">${sysIdx + 1}</th>
                        <td rowspan="${rowspan}" class="align-top p-4 border-r border-base-300 max-w-xs">
                            <div class="font-bold text-2xl text-primary mb-1">${system.main_system_name}</div>
                        ...
                        <td rowspan="${rowspan}" class="align-top text-center p-4 border-r border-base-300">
                            <div class="stat p-0">
                                <div class="stat-value text-secondary text-2xl">${system.total_users}</div>
                        ...
                        <td rowspan="${rowspan}" class="align-top p-4 border-r border-base-300 min-w-[140px]">
                            ...
                            <div class=" p-0 text-center">
                                <div class="stat-value text-error text-2xl">${system.unmatched}</div>
                    ...
                    <td class="p-4 border-b border-base-200">
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-wrap gap-1">
                                <div class="badge badge-success badge-outline text-[10px] py-2">Matched: ${matched + unmatched}</div>
                                <div class="badge badge-error badge-outline text-[10px] py-2">Unchecked: ${uncheck}</div>
                            </div>
                            <input type="hidden" name="programs[${program.program_id}][${program.name}][matched]" value="${matched}">
                            <input type="hidden" name="programs[${program.program_id}][${program.name}][unmatched]" value="${unmatched}">
                            <input type="hidden" name="programs[${program.program_id}][${program.name}][uncheck]" value="${uncheck}">
                        </div>
                    </td>
                    ...
                    <td class="p-4 border-b border-base-200">
                        <div class="flex flex-col gap-2">
                            <div class="join w-full">
                                <input type="number" name="programs[${program.program_id}][${program.name}][delete_count]" 
                                    class="input input-bordered input-sm join-item w-full focus:input-primary text-center" 
                                    placeholder="Delete" min="0">
                                <input type="number" name="programs[${program.program_id}][${program.name}][change_count]" 
                                    class="input input-bordered input-sm join-item w-full focus:input-primary text-center" 
                    ...
                    <td class="p-4 border-b border-base-200">
                        <textarea name="programs[${program.program_id}][${program.name}][detail_remark]" 
