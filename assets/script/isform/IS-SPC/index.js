...
$(document).ready(async function () {
	...
	async function fetchAdmins(platform, selector) {
		...
		const ssaEngineer = await $.get(
			`${host}isform/IS-SPC/main/get_SSA_engineer`
		);
		...
		if (filtered.length === 0) {
			...
			console.log(ssa);
			ssa.forEach((a) =>
				select.append(`<option value="${a.SEMPNO}">${a.SNAME}</option>`)
			);
			...
			filtered.forEach((a) =>
				select.append(
					`<option value="${a.EMPNO}">${a.USER_LOGIN} <span class="text-sm">(${a.USER_OWNER})</span></option>`
				)
			);
		}
	}
