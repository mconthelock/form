...
$(document).ready(async function () {
  ...
  if (dataForm && dataForm.length > 0) {
    ...
    if (firstItem.PERSON_IN_CHARGE || firstItem.EMPNAME || firstItem.CREATE_BY) {
      $("#person-in-charge").text(firstItem.PERSON_IN_CHARGE || firstItem.EMPNAME || firstItem.CREATE_BY || "-");
    }
    
    // Set Type with badge color
    const formType = firstItem.TYPE_NAME;
    const badgeClass = formType === "New Address" ? "badge-success" : "badge-info";
