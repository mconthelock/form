...
$(document).ready(async function () {
    ...
    $('.period').text(
        `${detail.ASSIGN?.YEAR ?? ''} half #${detail.ASSIGN?.PERIOD === '1' ? '1st' : '2nd'}`,
    );

    // ---- Fill Bulk / Stock summary boxes ----
    $('.total-items-bulk').text(
        formatNumber(detail.RESULT?.filter((row) => row.TYPE === '1').length),
    );
    $('.checking-items-bulk').text(formatNumber(detail.BULK_ITEM));
    $('.diff-items-bulk').text(
        formatNumber(
            detail.RESULT?.filter(
                (row) => row.TYPE === '1' && Number(row.ON_HAND) !== Number(row.ACTUAL_QTY),
            ).length,
        ),
    );
    $('.variance-items-bulk').text(
        formatNumber(
            detail.RESULT?.filter(
                (row) => row.TYPE === '1' && Number(row.ON_HAND) !== Number(row.ACTUAL_QTY),
            ).length,
    ...
    $('.total-items-stock').text(
        formatNumber(detail.RESULT?.filter((row) => row.TYPE === 'A').length),
    );
    $('.checking-items-stock').text(formatNumber(detail.STOCK_ITEM));
    $('.diff-items-stock').text(
        formatNumber(
            detail.RESULT?.filter(
                (row) => row.TYPE === 'A' && Number(row.ON_HAND) !== Number(row.ACTUAL_QTY),
            ).length,
        ),
    );
    $('.variance-items-stock').text(
        formatNumber(
            detail.RESULT?.filter(
                (row) => row.TYPE === 'A' && Number(row.ON_HAND) !== Number(row.ACTUAL_QTY),
            ).length,
    ...
    rows.forEach((row, idx) => {
        const diffAmount = Math.abs(
            (Number(row.ACTUAL_QTY) - Number(row.ON_HAND)) * Number(row.PRICE),
        );
        totalAmount += diffAmount;

        const whiInput =
            mode === '2'
                ? `<input class="input input-xs whi-reply" data-id="${row.ID ?? ''}" value="${row.WHI_REPLY ?? ''}">`
                : (row.WHI_REPLY ?? '');
        const purInput =
            mode === '2'
                ? `<input class="input input-xs pur-reply" data-id="${row.ID ?? ''}" value="${row.PUR_REPLY ?? ''}">`
                : (row.PUR_REPLY ?? '');
        ...
        $tbody2.append(`
            <tr>
                ...
                <td class="border border-gray-400 px-2 py-1 text-center">${row.TAG_NO ?? ''}</td>
                <td class="border border-gray-400 px-2 py-1">${row.ITEM_CODE ?? ''}</td>
                <td class="border border-gray-400 px-2 py-1">${row.ITEM.IDESC ?? ''}</td>
    ...
    console.log('Total Variance Amount:', totalAmount);
    $('.total-amount-1').text(formatNumber(totalAmount, 2));
    $('.total-amount-2').text(formatNumber(totalAmount, 2));
