...
const columns = [
	...
	},
	{
		...
		render: function (data, type, row) {
			...
			return `
      <div
        class="toggle-btn w-8 h-4 flex items-center rounded-full p-0.5 cursor-pointer transition-colors duration-300 ${
			data == 1 ? "bg-green-500" : "bg-gray-400"
		}"
        data-id="${row.GID}"
      >
        <div class="toggle-circle bg-white w-3 h-3 rounded-full shadow-md transform transition-transform duration-300 ${
			data == 1 ? "translate-x-4" : "translate-x-0"
		}" data-id="${row.GID}"></div>
      </div>
    `;
		},
	...
	$.ajax({
		...
		success: function (res) {
			if (res.status) {
				...
				$btn.find(".toggle-circle").toggleClass(
	...
	$.ajax({
		...
		success: function (res) {
			if (res && res.length > 0) {
				...
				row.find(".dds-col").text(
					data.SSEC + "/" + data.SDEPT + "/" + data.SDIV
				);
	...
	$.ajax({
		...
		success: function (res) {
          ...
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-3 py-2">
            <input type="text" name="pst[]" value="${
				p.EMAIL || ""
			}"  class="emailinp input input-bordered rounded-xl w-full shadow-sm border-green-200 text-gray-900" />
             </td>
            <td class="px-3 py-2 name-col">${p.SNAME || ""}</td>
            <td class="px-3 py-2 pos-col">${p.SPOSNAME || ""}</td>
            <td class="px-3 py-2 dds-col">${[p.SDIV, p.SDEPT, p.SSEC]
				.filter(Boolean)
				.join("/")}</td>
          </tr>`;
