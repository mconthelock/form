...
$(async function () {
    ...
    $tableBody.on('change', '.dietary_require', function () {
        ...
        if (val === 'Food Allergies' || val === 'Other') {
            ...
            if (customText) {
                ...
                if (
                    $select.find("option[value='" + newVal + "']").length === 0
                ) {
    ...
    $(document).on('click', '#addVisitorBtn', async function () {
        ...
        $newRow
            .find('td:first')
            .addClass($firstRow.find('td:first').attr('class'))
            .attr('tabindex', 0)
            .css('left', '0px');
    });
    ...
    await setSelect2({
        element: tableEmpBody.find('.dietary_require'),
        selectionCssClass: 'w-40',
        width: '100%',
    });
    ...
    $(document).on('change', '#visitor_file', async function (e) {
        ...
        $.each(data, function (index, row) {
            ...
            // No.
            tr.append(
                `<td class="px-2 py-2 sticky left-0 bg-white z-15">${
                    index + 1
                }</td>`,
            );
            ...
            tr.append(
                `<td class="px-2 py-2"><input type="text" name="country[]" value="${
                    row[0] || ''
                }" class="input input-bordered rounded-xl w-full"/></td>`,
            );
            tr.append(
                `<td class="px-2 py-2"><input type="text" name="company[]" value="${
                    row[1] || ''
                }" class="input input-bordered rounded-xl w-full"/></td>`,
            );
            tr.append(
                `<td class="px-2 py-2"><input type="text" name="name[]" value="${
                    row[2] || ''
                }" class="input input-bordered rounded-xl w-full"/></td>`,
            );
            tr.append(
                `<td class="px-2 py-2"><input type="text" name="pos[]" value="${
                    row[3] || ''
                }" class="input input-bordered rounded-xl w-full"/></td>`,
            );
            ...
            const expSelect = $(
                '<select name="exp[]" class="input input-bordered rounded-xl w-full"></select>',
            );
            ...
            const lunchSelect = $(
                '<select name="lunch_provided[]" class="input input-bordered rounded-xl w-full"></select>',
            );
            ...
            const dinnerSelect = $(
                '<select name="dinner_provided[]" class="input input-bordered rounded-xl w-full"></select>',
            );
            ...
            const dietarySelect = $(
                '<select name="dietary_require[]"  class="dietary_require input input-bordered rounded-xl w-full"></select>',
            );
            dietarySelect.append('<option value=""></option>');
            dietaryOptions.forEach((opt) =>
                dietarySelect.append(`<option value="${opt}">${opt}</option>`),
            );
...
$(async function () {
    ...
    Object.entries(sproj).forEach(([projno, items]) => {
        ...
        mappedItems.forEach((item) => {
            const expDate = item.EXPPLAN
                ? new Date(item.EXPPLAN).toLocaleDateString('en-GB')
                : '';
            console.log('xxxxxx');
            const detailHtml = `
    ...
    toggleColumnByHeader(
        ...
        $('#hasLunch').is(':checked') || $('#hasDinner').is(':checked'),
    );
    toggleColumnByHeader(
        'tableemp',
        'Dietary Requirements',
        ...
        data.forEach((item, idx) => {
            ...
            detailRows += `
              <tr class="detail-row bg-gray-50 text-gray-700">
                  ...
                  <td class="px-2 py-2">${
                      item.PRJ_NAME || ''
                  }<input type="hidden" name="sprojname_${
                      item.PRJ_NO
                  }[]" value="${item.PRJ_NAME}"></td>
                  <td class="px-2 py-2">${
                      item.MODEL || ''
                  }<input type="hidden" name="sprojmodel_${
                      item.PRJ_NO
                  }[]" value="${item.MODEL}"></td>
                  <td class="px-2 py-2">${
                      item.SPEC || ''
                  }<input type="hidden" name="sprojspec_${
                      item.PRJ_NO
                  }[]" value="${item.SPEC}"></td>
                  <td class="px-2 py-2">${
                      item.TOTUNIT || ''
                  }<input type="hidden" name="sprojqty_${
                      item.PRJ_NO
                  }[]" value="${item.TOTUNIT}"></td>
                  <td class="px-2 py-2">${expDate}<input type="hidden" name="sprojsta_${
                      item.PRJ_NO
                  }[]" value="${expDate}"></td>
              </tr>
          `;
...
$(document).on('click', '.del-btn', async function () {
    ...
    if (isConfirmed) {
        const resdel = await deleteForm({
            vmscyear2: $('#cyear2').val(),
            vmsnrunno: $('#nrunno').val(),
        });
        if (resdel.status) {
            const con = {
                NFRMNO: $('#nfrmno').val(),
...
$(document).on('click', '.save-btn', async function () {
    ...
    let isValid =
        tab === 'visitarg'
            ? validateVisitTab(form)
            : tab === 'req'
              ? validateReqItemTab(form)
              : tab === 'stk' ||
                  tab === 'sch' ||
                  tab === 'inf' ||
                  tab === 'meal' ||
...
$(document).on('click', '.edit-btn', function () {
    ...
    // reset active menu
    $('li button[data-tab]').removeClass(
        'active-tab border-blue-600 bg-white/30',
    );

    // set active ให้ menu ที่ตรงกับ tab
    $("li button[data-tab='tab-" + tab + "']").addClass(
        'active-tab border-blue-600 bg-white/30',
    );
});
...
async function createGPENT() {
    ...
    const fname =
        inputFiles.length > 0
            ? inputFiles[0].name
            : $('#attachmentfileDisplay .file-item').first().data('filename') ||
              null;
    ...
    const buildFormDataEnt = async (mealType) => {
        ...
        const purpose =
            $('#detail').val() !== '' ? $('#detail').val() : purposeText;
        ...
        let location =
            mealType === 'lunch'
                ? $('#lunch').find('option:selected').text()
                : 'Outside';
    ...
    if ($('#hasLunch').is(':checked')) {
        ...
        links.push({
            url:
                host +
                `gpform/GP-ENT/main?sr=4&no=${eno}&orgNo=${evorgno}&y=${ecyear}&y2=${ecyear2}&runNo=${enrunno}`,
        ...
        SaveVMSENT(
            $('#cyear2').val(),
            $('#nrunno').val(),
            lunchFormData.get('cyear2'),
            lunchFormData.get('nrunno'),
        );
    ...
    if ($('#hasDinner').is(':checked')) {
        ...
        SaveVMSENT(
            $('#cyear2').val(),
            $('#nrunno').val(),
            dinnerFormData.get('cyear2'),
            dinnerFormData.get('nrunno'),
        );
        if (companiesArr[0].fileName) {
            saveENTfile(
                $('#nfrmno').val(),
    ...
    links.forEach((link, index) => {
        field.append(
            `<a href="${link.url}" target="_blank" class="text-blue-600 hover:text-blue-800 underline">${link.text}</a>`,
        );
    ...
    $.ajax({
        ...
        error: function (xhr) {
            Swal.fire({
                ...
                text:
                    xhr.responseText ||
                    'Failed to create Entertainment Approval Request Form',
            });
        },
    });
}
...
function loaddata(vmscyear2, vmsnrunno) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (
                response.head.PURPOSEVISIT == null &&
                response.head.PURPOSEDETAIL == null
            ) {
                $('[data-field="purpose"]').text('');
            } else {
                $('[data-field="purpose"]').text(
                    response.head.PURPOSEVISIT +
                        ' ' +
                        response.head.PURPOSEDETAIL,
                );
            }
            ...
            let lname = parts[2] || '';
            let nameshort =
                title + ' ' + fname + ' ' + lname.charAt(0).toUpperCase() + '.';
            ...
            if (
                item.SFILE &&
                typeof item.SFILE === 'string' &&
                item.SFILE.trim() !== ''
            ) {
                $('[data-field="filename"]').html(`
          <a href="${host}marform/MAR-VMS/form/mdownload/${NFRMNO}_${VORGNO}_${CYEAR}_${vmscyear2}_${vmsnrunno}/${item.SFILE.substring(
              13,
          )}/${item.SFILE}"
            target="_blank"
            class="file-list">
        ...
        `);
            ...
            const formc1Val =
                item.FORMC1_1 === 'Y' ? 'Yes (only visit Test Tower)' : 'No';
        ...
        `;
          ...
          <tr>
            <td class="border px-2">${
                v.SCHSTIME && v.SCHETIME ? v.SCHSTIME + ' - ' + v.SCHETIME : '-'
            }</td>
      ...
      `;
        ...
        `;
            ...
            if (response.ent && response.ent.length > 0) {
                ...
                let html = response.ent
                    .map((e) => {
                        return (
                            `<a href="` +
                            host +
                            `gpform/GP-ENT/main?sr=4&no=9&orgNo=030101&y=25&y2=${e.ENTCYEAR2}&runNo=${e.ENTNRUNNO}" target="_blank" class="text-blue-600 hover:text-blue-800 underline">[${e.REQENT}]</a>`
                        );
                    })
                    .join(', ');
                // ใส่ใน span
                document.querySelector('[data-field="formreqent"]').innerHTML =
                    html;
            }
        },
        complete: function (xhr, status) {
