        ...
        <div class="rounded-2xl overflow-hidden shadow-sm border border-base-300 mb-8">
            <div class="bg-sky-600 text-white p-6">
                <h1 class="text-2xl md:text-3xl font-bold tracking-tight">
                    Regular Review <span class="mx-2 bg-white/15 px-2 py-1 rounded">({{ $program }}) </span> [{{ $formNumber }}]
                </h1>
                        ...
                        <table class="table table-sm border-collapse border" id="checktable">
                            <thead>
                                <tr class="text-center bg-base-200">
                                    ...
                                    <th class="border border-gray-300">Server Role</th>
                                    <th class="border border-gray-300">Database Role</th>
                                    <th class="border border-gray-300">Server</th>
                                    <th class="border border-gray-300">User Profile</th>
                                    <th class="border border-gray-300">Status</th>
                                    <th class="border border-gray-300">User Description</th>
                            ...
                            <tbody>
                                @foreach ($user as $key => $item)
                                    <tr class="text-center hover:bg-gray-200!">
                                        ...
                                        <td class="border border-gray-300 text-left">{{ $item->DESCRIPT }} <br> <span class="text-primary/70">( {{ $item->SNAME }} )</span></td>
                                        <td class="border border-gray-300">
                                            @if (is_null($item->RESULT))
                                                <input type="radio" id="correct-{{ $item->SEMPNO }}" data-userlogin="{{ $item->USER_LOGIN }}_{{ $item->SERVER_NAME }}" checked name="result[{{ $item->SEMPNO }}{{ $key }}]" class="radio result-radio radio-success" value="1">
                                            @else
                                                {!! $item->RESULT == '1' ? '&#x2714;' : '' !!}
                                            @endif
                                        </td>
                                        <td class="border border-gray-300">
                                            @if (is_null($item->RESULT))
                                                <input type="radio" id="incorrect-{{ $item->SEMPNO }}" data-userlogin="{{ $item->USER_LOGIN }}_{{ $item->SERVER_NAME }}" name="result[{{ $item->SEMPNO }}{{ $key }}]" class="radio result-radio radio-error" value="0">
                                            @else
                                                {!! $item->RESULT == '0' ? '&#x2714;' : '' !!}
                                            @endif
                                        </td>
