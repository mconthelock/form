...
@section('contents')
    @php
        ...
        foreach ($file_attach ?? [] as $file) {
            if (!empty($file->FILE_TYPE) && preg_match('/^MEMO_TYPE_(\d+)$/', $file->FILE_TYPE, $matches)) {
                $expenseMemoFiles[(int) $matches[1]][] = $file;
            } else {
                $generalAttachFiles[] = $file;
            }
        }
    @endphp
                    ...
                    @foreach ($typeConfigs as $typeId => $cfg)
                        @if (!empty($presentTypes[$typeId]))
                            ...
                            <div class="border-2 {{ $cfg['border'] }} rounded-2xl p-4 {{ $cfg['bg'] }} shadow-sm transition mb-8" data-estimate="{{ $estimateByType[$typeId] ?? 0 }}">
                                <table class="min-w-full text-sm border rounded-xl overflow-hidden expense-table-split" data-type="{{ $typeId }}" data-label="{{ $cfg['label'] }}">
                                    <thead>
                                ...
                                <div class="grid grid-cols-1 gap-6 mt-6 border-t {{ $cfg['borderLight'] }} pt-4 memo-section" id="memo-section-{{ $typeId }}" data-type="{{ $typeId }}" data-has-existing-memo="{{ !empty($expenseMemoFiles[$typeId]) ? '1' : '0' }}"
                                    ...
                                    <div>
                                        ...
                                        @if (!empty($expenseMemoFiles[$typeId]))
                                            <div class="mt-4 space-y-1">
                                                @foreach ($expenseMemoFiles[$typeId] as $file)
                                                    <div class="flex items-center justify-between gap-3 border rounded-lg p-2 bg-white">
            ...
            @else
                <div class="mt-10">
                    ...
                    <div class="border-2 border-green-500 rounded-2xl p-4 bg-green-50 shadow-sm transition">
                        <table class="min-w-full text-sm border rounded-xl overflow-hidden" id="expense-table">
                            <thead>
            ...
            <div class="mt-8">
                <div class="bg-green-50 rounded-2xl border-2 border-green-500 p-6 shadow space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        @if (!$splitExpense || !empty($generalAttachFiles))
                            <div>
                                <label class="font-semibold text-green-700 mb-1 block">Attach Memo{{ $splitExpense ? ' (General)' : '' }}:</label>
                                <div class="flex items-center gap-3">
                                ...
                                @if (!empty($generalAttachFiles))
                                    <div class="mt-6 space-y-1">
                                        @foreach ($generalAttachFiles as $file)
