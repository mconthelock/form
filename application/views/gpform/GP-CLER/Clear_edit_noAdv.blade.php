        ...
        <div class="max-w-6xl mx-auto bg-white rounded-3xl shadow-lg p-6 md:p-10 space-y-10 border border-blue-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="input-by" class="input input-bordered rounded-xl w-full shadow-sm border-blue-200" value="{{ $ENT_FORM->EMP_INPUT }}" placeholder="Input Employee Code" />
                </div>
                <div>
                    <label class="block mb-1 font-semibold text-blue-700">Requested by</label>
                    <input type="text" id="requested-by" class="input input-bordered rounded-xl w-full shadow-sm border-blue-200" value="{{ $ENT_FORM->EMP_REQ }}" placeholder="Input Employee Code" />
                </div>
                <div>
                    <label class="block mb-1 font-semibold text-blue-700">Entertainment Date</label>
                    <input type="date" id="entertain-date" class="input input-bordered rounded-xl w-full shadow-sm border-blue-200" value="{{ date('Y-m-d', strtotime($ENT_FORM->ENTERTAINMENT_DATE)) }}" />
                </div>
                <div></div>
                <div class="md:col-span-2">
                    <label class="block mb-1 font-semibold text-blue-700">*Purpose for Entertainment</label>
                    <textarea id="purpose" class="textarea input-bordered rounded-xl w-full min-h-[48px] shadow-sm border-blue-200" required>{{ $ENT_FORM->PURPOSE }}</textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="flex gap-6">
                        <label class="inline-flex items-center space-x-2">
                            <input type="radio" name="time" id="time-lunch" class="radio radio-primary" value="Lunch" {{ $ENT_FORM->TYPE_TIME == 'Lunch' ? 'checked' : '' }} />
                            <span>Lunch</span>
                        </label>
                        <label class="inline-flex items-center space-x-2">
                            <input type="radio" name="time" id="time-dinner" class="radio radio-primary" value="Dinner" {{ $ENT_FORM->TYPE_TIME == 'Dinner' ? 'checked' : '' }} />
                            <span>Dinner</span>
                        </label>
                    </div>
                </div>
                <div>
                    ...
                    <div class="flex flex-wrap gap-6 mb-2">
                        <label class="inline-flex items-center space-x-2">
                            <input type="radio" name="location" id="location-inside" class="radio radio-primary" value="Inside" {{ $ENT_FORM->LOCATION_TYPE == 'Inside' ? 'checked' : '' }} />
                            <span>Inside</span>
                        </label>
                        <label class="inline-flex items-center space-x-2">
                            <input type="radio" name="location" id="location-outside" class="radio radio-primary" value="Outside" {{ $ENT_FORM->LOCATION_TYPE == 'Outside' ? 'checked' : '' }} />
                            <span>Outside</span>
                        </label>
                    </div>
                    <input type="text" class="input input-bordered rounded-xl w-full shadow-sm border-blue-200" id="location_detail" value="{{ $ENT_FORM->LOCATION }}" placeholder="*Please identify the location." />
                </div>
            </div>
                    ...
                    <table class="table table-xs md:table-sm w-full border rounded-xl overflow-hidden">
                        ...
                        <tbody class="bg-amber-50">
                            @foreach ($guest_type as $gt)
                                <tr>
                                    <td><input type="radio" name="guest_type" class="checkbox bg-white checkbox-primary guest_type" value="{{ $gt->GT_ID }}" {{ $ENT_FORM->GUEST_TYPE == $gt->GT_ID ? 'checked' : '' }}></td>
            ...
            <div class="bg-gradient-to-r from-blue-50 via-white to-blue-100 rounded-2xl border border-blue-200 p-4 space-y-2">
                ...
                <div>
                    ...
                    <textarea class="textarea textarea-bordered rounded-xl w-full min-h-[40px]" id="remark" placeholder="เช่น ระบุเหตุผล...">{{ $ENT_FORM->REMARK }}</textarea>
                </div>
                    ...
                    <div class="bg-green-50 rounded-2xl border border-green-200 p-6 shadow space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div>
                                ...
                                <div class="flex gap-6 mt-1">
                                    <label class="inline-flex items-center space-x-2">
                                        <input type="radio" name="president_join" id="president_join" class="radio radio-success" value="1" {{ $formCler->PRESIDENT_JOIN == '1' ? 'checked' : '' }} required>
                                        <span>Join</span>
                                    </label>
                                    <label class="inline-flex items-center space-x-2">
                                        <input type="radio" name="president_join" id="president_join" class="radio radio-success" value="2" {{ $formCler->PRESIDENT_JOIN == '2' ? 'checked' : '' }} required>
                                        <span>Not Join</span>
                                    </label>
                                </div>
                            ...
                            <div>
                                ...
                                <input type="number" class="input input-bordered rounded-xl w-full border-green-200 text-lg" value="{{ $formCler->ACTUAL_COST }}" placeholder="ใส่ยอดค่าใช้จ่ายจริง" required id="actual-cost">
                            </div>
                            <!-- Remain -->
                        ...
                        <div class="grid grid-cols-2">
                            <div>
                                ...
                                <div class="mt-8">
                                    @if ($formCler->MEMO_FILE)
                                        <label>Current File : </label>
                                        <a href="{{ base_url('gpform/GP-CLER/main/preview/') . $formCler->MEMO_FILE }}" class="link btn rounded-lg text-sm text-blue-600 font-semibold" target="_blank">View Current</a>
                                    @endif
                                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-2">
                            <div>
                                ...
                                <textarea name="" id="reason" class="textarea rounded-xl w-full min-h-[44px]" id="reason" placeholder="เหตุผลว่าทำไมถึงไม่สามารถทำการขอ Entertainment ล่วงหน้า">{{ $formCler->REASON }}</textarea>
                            </div>
