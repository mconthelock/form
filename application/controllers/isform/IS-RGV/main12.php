...
class Main extends MY_Controller
{
    ...
    public function JobsCreateRGV()
    {
        ...
        foreach ($programList as $programItem) {
            ...
            switch ($programName) {
                ...
                case 'wsd':
                    $userList = $this->rm->wsd_user();
                    $userIds = array_column($userList, 'EMPNO');
                    break;
    ...
    public function testwsd()
    {
        $userList = $this->rm->is_user('AAS');
        $userLogins = array_column($userList, 'USER_LOGIN');
        $aa = array_map('trim', $userLogins);
        // $userList = $this->rm->as400_user();
        // $userIds  = array_map('trim', array_column($userList, 'EMPNO'));
        pre_array($aa);
    ...
    public function Update_Result()
    {
        ...
        foreach ($post as $key => $value) {
            ...
            $this->rm->update('ISRGV_EMP', $data, $where, 'UPDATE_AT');
