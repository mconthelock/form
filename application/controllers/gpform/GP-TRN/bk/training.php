...
class Training extends MY_Controller {
    ...
    public function return_to(){ // return to Training Officer exdata = '19'
    ...
    public function save_formcreate_clearance(){
        header('Content-Type: application/json; charset=utf-8');
        try {
            $data = $this->input->post();
            $this->trn->update_flow($data["NFRMNO"], $data["VORGNO"], $data["CYEAR"], $data["CYEAR2"], $data["NRUNNO"], 'CEXTDATA', '99', 'CSTEPNO', '--');
            ...
            $this->trn->update_flow($data["NFRMNO"], $data["VORGNO"], $data["CYEAR"], $data["CYEAR2"], $data["NRUNNO"], 'VREPNO', '15199', 'CEXTDATA', '18');
            $this->trn->update_flow($data["NFRMNO"], $data["VORGNO"], $data["CYEAR"], $data["CYEAR2"], $data["NRUNNO"], 'VREPNO', '15199', 'CEXTDATA', '19');
            $formno = $this->toFormNumber($data["NFRMNO"], $data["VORGNO"], $data["CYEAR"], $data["CYEAR2"], $data["NRUNNO"]);
            ...
            $result = $this->trn->insert_data("GP_CLRTRN_HEAD", $insert);
            ...
            if (!$result) {
                ...
                echo json_encode([
                    "status" => "error",
                    "message" => "Insert Clearance Training failed",
                    "db_error" => $db_error,
                    "received" => $data
                ]);
                return;
            ...
            echo json_encode(["status" => "success", "message" => "Insert CLRTRN successful", "received" => $data]);
        } catch (Exception $e) {
            //delete form & flow when error
            //$this->trn->delete_all_table($data["NFRMNO"], $data["VORGNO"], $data["CYEAR"], $data["CYEAR2"], $data["NRUNNO"], 'FLOW');
            //$this->trn->delete_all_table($data["NFRMNO"], $data["VORGNO"], $data["CYEAR"], $data["CYEAR2"], $data["NRUNNO"], 'FORM');
    ...
    public function create_cash_adv(){
        ...
        $insert_list_clear = array_merge($base_clear_adv, [
            "ID"      => '1',
            "NNOADV"  => $cashHead["NRUNNO"],
            "HOLDTAX" => '0'
        ]);
        $this->trn->insert_data('CLRLIST', $insert_list_clear);
        */
